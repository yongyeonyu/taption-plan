...
- 결정(2026-09-26): 집 성장 타마고치. 실제 기록이 있고 해당 현지 날짜 자정까지 미확인 0개인 날에 하루 +1레벨(하루 최대 1), 늦은 확인은 소급하지 않는다. 매년 Lv.1 모닥불부터 시작하며 평년 365·윤년 366까지, 전년도 집은 보관한다. 7일 연속 달성마다 진화 선택·연결 가능한 땅 조각·진화 테마 화랑이 액세서리를 지급한다. 액세서리는 보관함에서 착용한다.
- 진화 후보는 규모 단계(작은 거점→건물→랜드마크)를 유지하며 매주 전 세계 후보 3개 중 사용자가 고른다. 국가별 대표 랜드마크 5개(건축·자연·문화 상징)를 목표로 하고, 땅 조각은 직접 연결 배치한다. 윤년 2월 29일에는 Lv.366 그림에 한정 장식을 표시한다.
- 구현/검증 세부는 HME0926A01에서 추적한다. 앱타깃 빌드와 회귀 통과 전 완료 처리하지 않는다.
...
## HME0926A01 · 연간 집 성장·세계 랜드마크 진화 [부분 구현·콘텐츠/UI 미완료]

- 원인/목표: 매일 실제 일과를 완성할 이유와 성장 시각 보상이 필요하다. 기존 육각 격자는 폐기됐으므로 집·연결 땅·화랑이 수집으로 구성한다.
- 해결: 현지 날짜의 완료 실제 기록 ≥1 및 미확인 0일 때 하루 +1레벨, 자정 마감 후 확인은 미반영. 연초 Lv.1 모닥불로 새 시즌 시작, 과거 집 보관. 7일 연속 보상은 사용자 선택 진화(세계 후보 3개, 규모 단계 유지), 연결 배치 땅 조각, 테마 액세서리. 평년/윤년용 366개 단계 그림과 2월 29일 한정 장식 준비. 국가별 후보 목표 5개(건축·자연·문화), 갈래 요소는 기본 단계 그림과 조합.
- 구현: 현지 날짜별 실제 기록·미확인 시간 조건, 일일 레벨/연속일·주간 보상 데이터, 연도별 archive, 홈 레벨 핀 및 윤달 표시, 하루요약 시트에 걸음 수, 선택 시각 주변 시간축 카드, 366개 기본 SVG 그림과 데이터 정책 단위 테스트를 추가했다.
- 미완료: 주간 보상 선택·연결 땅 배치·액세서리 보관함의 사용자 UI, 실제 전 세계 랜드마크 카탈로그/그림, 연도별 archive 열람. 성장 화면의 실기기 확인도 대기. 366개는 기본 성장 단계 그림이며 국가별 랜드마크 콘텐츠가 아니다.
- 검증: `MapHomeTimeRailCardTests` 및 `MapHomeGrowthPolicyTests` 통과; SVG/Contents.json 366/366 유효성 확인; iOS generic Debug 빌드는 아래 최종 로그 기록 예정. 실기기 확인 전 기능 완료로 처리하지 않는다.
