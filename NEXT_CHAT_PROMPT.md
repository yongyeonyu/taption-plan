...
- GPS로 양 끝이 확인되는 누락 구간만 예상 경로를 생성하도록 변경했다. 장소 기반 전체 예상 경로와 미확정 지하철 전체 노선 표시를 제거하고 WBS 재생도 같은 공백 요청을 사용한다. 복수 공백 ID/부모 이동 ID 분리, 날짜 저장 왕복 안정성, v5 지도 캐시 적용. 정상 GPS·확정 경로·원본은 유지한다.
- main 작업 트리의 변경이며 새 버전 업로드 요청은 없었다. 배포된 148에는 이 수정이 없다. 다음 배포 시 빌드 번호를 올리고 별도 TestFlight 절차를 수행한다. 현재 git 상태를 확인하고 이 변경을 덮어쓰지 않는다.
- 최종 전체 회귀 1,113 PASS·기존 StoreKit 1 SKIP·0 FAIL. Debug 결과와 세부 검증은 test.md의 TP0913E001 및 build/validation/TP0913E001을 확인한다. 실제 지도 표시/터치 검증은 자동 테스트와 구분한다.
