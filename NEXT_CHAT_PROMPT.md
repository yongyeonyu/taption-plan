...
- 요청 ID: `REL903TF01`
- 배포 소스 SHA(바이너리): `cceb9f655c76b4e5981891ed7f80c89f03b6a2cc`
- 앱·iOS Widget·Watch 앱·Watch Widget: `1.0 (131)`
- Release archive: `/private/tmp/taption-rel903-build131/TaptionPlan-1.0-131.xcarchive`
- Export IPA: `/private/tmp/taption-rel903-build131/Export/TaptionPlan.ipa`
- IPA SHA-256: `4435667dba069fda3706217b5c6af8947cef75eb72689525bc2fd75dd603f6f8`
- Delivery/build UUID: `805169b0-e3d9-41b2-883d-475504a3884d`
- App Store Connect App ID: `6797370230`
- Internal 그룹: `TP Taption Plan 내부 테스트`, ID `b4857e5e-d1ff-4bc2-b9ad-a69bcd4603fd`
- build 131은 App Store Connect API에서 `VALID`·`expired=false`, 내부 그룹 빌드 API에 포함, 전체 그룹 빌드 92개, 내부 테스터 1명(`INSTALLED`)으로 readback했다.

## 현재 검증·정리 상태

- `test.md`에 build 131 회귀 테스트·archive/export·서명·API readback 증적이 있다.
- App Store Connect Chrome/in-app Browser의 build 131 그룹 빌드·테스터 화면 UI readback은 별도 게이트다. 계정 자격 증명을 대신 입력하지 않는다.
...
1. 로그인된 App Store Connect Chrome에서 internal 그룹의 build 131 빌드 탭과 테스터 탭을 열어 UI 노출을 readback한다.
2. iPhone·iPad의 TestFlight에서 build 131을 다운로드·설치한 뒤 version/build readback과 launch, Watch 설정 위치·비이동 졸라맨 정지·날짜 변경 시 사이드바 대분류 유지 화면을 검증한다. iPhone에서 `수면 연결`을 탭해 수면 읽기 권한을 승인하고 오늘 수면 actual 생성·iCloud 재백업을 확인한다.
