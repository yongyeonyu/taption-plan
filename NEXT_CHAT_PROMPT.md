...
- 요청 ID: `LOG904TF01`
- 배포 소스 SHA(바이너리): `f3425f951d9b05e67be39fcab62847244e77d31f`
- 앱·iOS Widget·Watch 앱·Watch Widget: `1.0 (132)`
- Release archive: `/private/tmp/taption-rel904-build132/TaptionPlan.xcarchive`
- Export IPA: `/private/tmp/taption-rel904-build132/Export/TaptionPlan.ipa`
- IPA SHA-256: `afc0a852750c0b67e017453a5b9c3aea05cf6b41bb89be211185496ac4de0b5b`
- Delivery/build UUID: `5d01450c-2c10-4b8f-bcff-8852258fe2f1`
- App Store Connect App ID: `6797370230`
- Internal 그룹: `TP Taption Plan 내부 테스트`, ID `b4857e5e-d1ff-4bc2-b9ad-a69bcd4603fd`
- build 132는 App Store Connect API에서 `VALID`·`APP_STORE_ELIGIBLE`·`expired=false`, 내부 그룹 빌드 API에 포함, 전체 그룹 빌드 93개, 내부 테스터 1명(`INSTALLED`)으로 readback했다.

## 현재 검증·정리 상태

- `test.md`에 build 132 집중 테스트·archive/export·검증·API readback 증적이 있다.
- App Store Connect Chrome/in-app Browser의 build 132 그룹 빌드·테스터 화면 UI readback은 별도 게이트다. 계정 자격 증명을 대신 입력하지 않는다.
...
1. 로그인된 App Store Connect Chrome에서 internal 그룹의 build 132 빌드 탭과 테스터 탭을 열어 UI 노출을 readback한다.
2. iPhone·iPad의 TestFlight에서 build 132를 다운로드·설치한 뒤 version/build readback과 launch, Watch 설정 위치·비이동 졸라맨 정지·날짜 변경 시 사이드바 대분류 유지 화면을 검증한다. 지하철 이동과 수면 권한·조회 뒤 iCloud 로그를 올려 이동 판정·예상경로·HealthKit 이벤트를 확인한다.
